...
namespace UtilityTools.Modules.ImageMagic.Model
{
    public class ZemMetaData : BindableBase
    {
        ...
        private string _detector = string.Empty;
    ...
    class ImageMetadata
    {
        ...
        {
            var descriptions = directories.Where(d => d.Name == "PNG-tEXt" && d.Tags?.Count > 0).Select(d => d.Tags![0].Description).ToList();
            ...
            return metadata ?? new ZemMetaData();
        }
        ...
        public static ZemMetaData ParseMetadataZem20(IReadOnlyList<Directory> directories)
        {
            ...
            Dictionary<string, string> dict = new Dictionary<string, string>()
            {
                ...
                {"detector", d["Detecter"]},
                {"Ob", d["OB"][0..^2]},
                {"zoom", d["Mag"][0] == 'X'? d["Mag"][1..] : d["Mag"][0..]},
